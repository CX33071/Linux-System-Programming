...
//      }
//     public:
//     Server(uint16_t port):port_(port),serversocket_(AF_INET,SOCK_STREAM,0){
...
class Socket{
    private:
     ...
     bool enableReuseAddr() { int opt = 1;
    if(::setsockopt(fd_,SOL_SOCKET,SO_REUSEADDR,&opt,sizeof(opt))==-1){
        return false;
    }
    return true;
...
class EPOLLserver {
    private:
     ...
     uint16_t port_;
     public:
     EPOLLserver(uint16_t port):port_(port),serversocket_(AF_INET,SOCK_STREAM,0){
         IPv4Address addr(port_);
         bind(serversocket_.fd(), addr.change(), sizeof(addr));
         ...
         while (1) {
             ...
             for (int i = 0; i < n;i++){
                if(EVENTS[i].data.fd==serversocket_.fd()){
                    ...
                    ssize_t n;
                    char buf[1024];
                    while( n = (recv(serversocket_.fd(), buf,sizeof(buf),0))>0){
                        send(serversocket_.fd(), buf, n,0);
                    }
                }
             }
         }
